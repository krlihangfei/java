...
public class TaskDAOImpl implements ITaskDAO {
	...
	public Task findById(long taskid) {
		...
		**/
		...
		Object[] params = new Object[]{taskid};  //new:代表是动态初始化; Object[] params = new Object[]{ taskid } : 等同于根据传入的taskid 创建多个对象, 组成一个数组
		/**
		正常的写法是
		Object[] params;//定义
		...
		**/
		JDBCHelper jdbcHelper = JDBCHelper.getInstance(); //创建连接池单例
		jdbcHelper.executeQuery(sql, params, new JDBCHelper.QueryCallback() {  //第三个参数是接收一个查询请求结果
		// 创建了一个JDBCHelper对象, 并调用该类的接口方法QueryCallback,
			
			@Override // 重写 查询结果的处理方法 --> 将查询结果的相关数据赋值给相应的变量
			public void process(ResultSet rs) throws Exception {
				if(rs.next()) {
					...
					task.setTaskid(taskid);   //然后将变量的值通过task的javabean方法,传递给task类的相应字段.
